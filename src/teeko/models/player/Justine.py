...
class Justine(IAPlayer):
    ...
    def eval(board: Board, color: TeekoColorEnum) -> float:
        ...
        if board.is_game_over():
            ...
            value = 40*Justine.piece_distance_from_center_coef(player_position) + 60*Justine.piece_distance_from_center_coef(opponent_position)
            # board.display()
            # print("value: ", value)
    ...
    def move(self, board: Board, color: TeekoColorEnum) -> Movement:
        best_value = Justine.minmax(board, color, 4, float("-inf"), float("inf"))
