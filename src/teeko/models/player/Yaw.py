...
class Yaw(AIPlayer):
    ...
    def eval(self, board: Board) -> float:
        ...
        value = 40 * Yaw.piece_distance_from_center_coef(
            my_positions) + 60 * Yaw.piece_distance_togehter_coef(
                my_positions)
    ...
    def minmax(self, board: Board, color: TeekoColorEnum, depth: int) -> float:
    ...
    def move(self, board: Board, color: TeekoColorEnum) -> Movement:
        ...
        best_value = self.minmax(board, color, deep)
        ...
        while len(movements) > 0:
            ...
            next_value = self.minmax(next_board, get_opponent(color), deep - 1)
